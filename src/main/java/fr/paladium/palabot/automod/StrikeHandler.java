 ...
 */
package fr.paladium.palabot.automod;

import fr.paladium.palabot.Action;
import fr.paladium.palabot.Constants;
import fr.paladium.palabot.Vortex;
import fr.paladium.palabot.database.managers.PunishmentManager.Punishment;
import fr.paladium.palabot.utils.FormatUtil;
import fr.paladium.palabot.utils.LogUtil;
import fr.paladium.palabot.utils.OtherUtil;
    ...
    public void applyStrikes(Member moderator, OffsetDateTime nowo, User target, int number, String reason)
    {
        ...
        else
        {
            ...
            if(punishments.stream().anyMatch(p -> p.action==Action.BAN) && canBan)
            {
                ...
                vortex.getDatabaseMongo().tempbans.clearBan(moderator.getGuild(), target.getIdLong());
                return;
            }
            int muteDuration = 0;
            int banDuration = 0;
            ...
            if(banDuration>0 && canBan)
            {
                ...
                vortex.getDatabaseMongo().tempbans.setBan(moderator.getGuild(), target.getIdLong(), now.plus(banDuration, ChronoUnit.MINUTES), reason, false);
                if(muteDuration>0)
                    vortex.getDatabaseMongo().tempmutes.setMute(moderator.getGuild(), target.getIdLong(), muteTime(now, muteDuration));
                return;
            }
