...
import fr.paladium.palabot.utils.ArgsUtil.ResolvedArgs;
    ...
    protected void execute(CommandEvent event)
    {
        ...
        for(int i=0; i<ids.size(); i++)
        {
            ...
            event.getGuild().ban(id, daysToDelete, reason).queue(success -> 
            {
                ...
                if(minutes>0)
                    vortex.getDatabaseMongo().tempbans.setBan(event.getGuild(), uid, unbanTime, reason, false);
                else
                    vortex.getDatabaseMongo().tempbans.setBan(event.getGuild(), uid, unbanTime, reason, true);
                if(last)
                    event.reply(builder.toString());
            }, failure -> 
            {
