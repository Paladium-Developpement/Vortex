 ...
 */
package fr.paladium.palabot.commands.settings;
...
import fr.paladium.palabot.database.managers.AutomodManager.AutomodSettings;
...
public class SetupCmd extends Command
{
    ...
    public SetupCmd(Vortex vortex)
    {
        ...
        this.children = new Command[]{new MuteSetupCmd(), new AutomodSetupCmd(), new GroupSetupCmd()};
    }
    ...
    private class AutomodSetupCmd extends Command
    {
        ...
        protected void execute(CommandEvent event)
        {
            waitForConfirmation(event, "This will enable all automod defaults. Any existing settings will not be overwritten.", () -> 
            {
                ...
                sb.append("\n").append(event.getClient().getWarning()).append(" Any settings not shown here were not set due to being already set.");
                event.replySuccess(sb.toString());
            });
        }
    }

    private class GroupSetupCmd extends Command
    {
        private GroupSetupCmd()
        {
            this.name = "group";
            this.category = new Category("Settings");
            this.help = "sets up the group permission";
            this.guildOnly = true;
            this.userPermissions = new Permission[]{Permission.MANAGE_SERVER};
            this.botPermissions = new Permission[]{Permission.ADMINISTRATOR};
            this.cooldown = 20;
        ...
        @Override
        protected void execute(CommandEvent event) {
            StringBuilder sb = new StringBuilder(event.getClient().getSuccess()+" Groups setup started!\n");
            event.reply(sb + Constants.LOADING+" Initializing Groups...", m -> event.async(() ->
            {
                int groupadded = vortex.getDatabaseMongo().groups.initGuild(event.getGuild());
                sb.append(event.getClient().getSuccess()).append(groupadded + " Groups initialized!\n");
                m.editMessage(sb).complete();
