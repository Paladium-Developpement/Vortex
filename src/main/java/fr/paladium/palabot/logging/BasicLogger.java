 ...
 */
package fr.paladium.palabot.logging;

import fr.paladium.palabot.Vortex;
import fr.paladium.palabot.logging.MessageCache.CachedMessage;
import fr.paladium.palabot.utils.FormatUtil;
import fr.paladium.palabot.utils.LogUtil;
import fr.paladium.palabot.utils.Usage;
...
public class BasicLogger
{
    ...
    private final static String DELETE = "<:cancel:835544809547890728>"; // ❌
    ...
    public void logMessageDelete(CachedMessage oldMessage)
    {
        ...
        if(formatted.isEmpty())
            ...
            String user = author == null ? FormatUtil.formatCachedMessageFullUser(oldMessage) : FormatUtil.formatFullUserId(author.getIdLong());
            ...
            buildr.setTitle(DELETE + vortex.getTraduction().getString("logger.message.deleted"));
            buildr.setDescription(vortex.getTraduction().getString("user") + ":" + user + " | " + vortex.getTraduction().getString("channel") + ":" + mtc.getAsMention());
            buildr.addField(vortex.getTraduction().getString("logger.message.deleted"), formatted, false);
            buildr.setFooter("ID : " + author.getId() + " | Date : " + OffsetDateTime.now().format(pattern));
            tc.sendMessage(buildr.build()).queue();
    }
    ...
    public void logMessageBulkDelete(List<CachedMessage> messages, int count, TextChannel text)
    {
        ...
        vortex.getTextUploader().upload(LogUtil.logCachedMessagesForwards("Deleted Messages", messages, vortex.getShardManager()), "DeletedMessages", (view, download) ->
        {
            ...
            EmbedBuilder buildr = new EmbedBuilder();
            buildr.setTitle(BULK_DELETE + vortex.getTraduction().getString("logger.message.deleted"));
            buildr.setDescription(count + vortex.getTraduction().getString("logger.message.bulkdeleted") + " " + vortex.getTraduction().getString("in") + " " + text.getAsMention());
            buildr.addField(VIEW + vortex.getTraduction().getString("logger.message.view"), view, false);
            buildr.addField(DOWNLOAD + vortex.getTraduction().getString("logger.message.download"), download, false);
            buildr.setFooter("Date : " + OffsetDateTime.now().format(pattern));
            tc.sendMessage(buildr.build()).queue();
